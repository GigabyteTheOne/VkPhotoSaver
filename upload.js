...
window.onload = function() {
	...
	chrome.storage.local.get('gid', function(result) {
		var value = result.gid || "";
		document.getElementById("gidInput").value = value;
	});
	chrome.storage.local.get('aid', function(result) {
		var value = result.aid || "";
		document.getElementById("albumIdInput").value = value;
	});
	chrome.storage.local.get('secondGid', function(result) {
		var value = result.secondGid || "";
		document.getElementById("secondGidInput").value = value;
	});
	chrome.storage.local.get('secondAid', function(result) {
		var value = result.secondAid || "";
		document.getElementById("secondAlbumId").value = value;
	});
	chrome.storage.local.get('offset', function(result) {
		var value = result.offset || 0;
		document.getElementById("offsetInput").value = value;
	});
	chrome.storage.local.get('limit', function(result) {
		var value = result.limit || 0;
		document.getElementById("limitInput").value = value;
	});
...
function showPhotoLinks(event) {
	...
	request("GET", url, function(event) {
		...
		if (answer.response) {
			...
			resultDiv.innerHTML = "Найдено " + data.length + " фото<br>";
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (imageSrc) {
					textarea.value += imageSrc + String.fromCharCode(13);
				}
			}
